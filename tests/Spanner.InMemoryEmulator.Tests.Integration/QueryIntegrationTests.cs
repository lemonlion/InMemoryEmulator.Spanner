...
public class QueryIntegrationTests
{
	...
	public QueryIntegrationTests(EmulatorSession session)
	{
		_fixture = TestFixtureFactory.Create(session);
	}

	private void SetupSingersTable(string tableName = "Singers")
	{
		var db = _fixture.Database!;
		db.ExecuteDdl($"CREATE TABLE {tableName} (SingerId INT64 NOT NULL, Name STRING(MAX), Age INT64) PRIMARY KEY (SingerId)");
		db.Insert(tableName, new Dictionary<string, object?> { ["SingerId"] = 1L, ["Name"] = "Alice", ["Age"] = 30L });
		db.Insert(tableName, new Dictionary<string, object?> { ["SingerId"] = 2L, ["Name"] = "Bob", ["Age"] = 25L });
		db.Insert(tableName, new Dictionary<string, object?> { ["SingerId"] = 3L, ["Name"] = "Charlie", ["Age"] = 35L });
	}
	...
	public async Task SelectNullColumn_ReturnsDBNull()
	{
		var table = "Q_Null";
		var db = _fixture.Database!;
		db.ExecuteDdl($"CREATE TABLE {table} (Id INT64 NOT NULL, Val STRING(MAX)) PRIMARY KEY (Id)");
		db.Insert(table, new Dictionary<string, object?> { ["Id"] = 1L, ["Val"] = null });
